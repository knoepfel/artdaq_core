...
void artdaq::configureTRACE(fhicl::ParameterSet& trace_pset)
{
	...
	*/
	...
	for (auto name : names)
	{
		if (name == "TRACE_NUMENTS" || name == "TRACE_ARGSMAX" || name == "TRACE_MSGMAX" || name == "TRACE_FILE")  // only applicable if env.var. set before before traceInit
