	...
	, artdaq_Fragment_(f)
{
	...
	{
		TLOG(TLVL_ERROR) << "ContainerFragmentLoader: Raw artdaq::Fragment object size suggests it does not consist of its own header + the ContainerFragment::Metadata object" ;
		TLOG(TLVL_ERROR) << "artdaq_Fragment size: " << artdaq_Fragment_.size() << ", Expected size: " << artdaq::detail::RawFragmentHeader::num_words() + words_to_frag_words_(Metadata::size_words) ;
...
inline void artdaq::ContainerFragmentLoader::addSpace_(size_t bytes)
{
	...
	TLOG(TLVL_TRACE) << "addSpace_: dataEnd_ is now at " << (void*)dataEnd_();
}
...
inline void artdaq::ContainerFragmentLoader::addFragment(artdaq::Fragment& frag)
{
	...
	{
		TLOG(TLVL_ERROR) << "addFragment: Fragment is full, cannot add more fragments!" ;
		throw cet::exception("ContainerFull") << "ContainerFragmentLoader::addFragment: Fragment is full, cannot add more fragments!";
	}

	TLOG(TLVL_TRACE) << "addFragment: Adding Fragment with payload size " << frag.dataSizeBytes() << " to Container";
	...
	{
		TLOG(TLVL_ERROR) << "addFragment: Trying to add a fragment of different type than what's already been added!" ;
	...
	TLOG(TLVL_TRACE) << "addFragment: Payload Size is " << artdaq_Fragment_.dataSizeBytes() << ", lastFragmentIndex is " << lastFragmentIndex() << ", and frag.size is " << frag.sizeBytes();
	...
	TLOG(TLVL_TRACE) << "addFragment, copying " << frag.sizeBytes() << " bytes from " << (void*)frag.headerAddress() << " to " << (void*)dataEnd_();
